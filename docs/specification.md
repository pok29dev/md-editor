...
**MD Editor** — Desktop application สำหรับ browse, view และ edit ไฟล์ Markdown และไฟล์ structured data (JSON/YAML)

### 1.2 วัตถุประสงค์

สร้าง desktop app ที่ใช้งานง่าย เน้นการทำงานกับไฟล์ `.md` และ config/data files (`.json`, `.yaml`, `.yml`) ในเครื่อง local โดยมี layout คล้าย **CodeRunner**:
...
| **Tab Bar** | เปิดหลายไฟล์พร้อมกัน แสดง unsaved indicator (`•`); คลิกขวา → Close All Tabs |
...
| F-02 | แสดง file tree ด้านซ้าย กรอง `.md`, `.markdown`, `.mdown`, `.mkd`, `.mdx`, `.json`, `.yaml`, `.yml` | Must |
...
| F-09 | New Window — workspace แยกต่อหน้าต่าง (`⌘⇧N`) | Must |
| F-10 | Close All Tabs จาก context menu บนแท็บ | Should |
...
| E-08 | แก้ไข JSON/YAML พร้อม syntax highlighting | Must |
| E-09 | ตรวจ syntax JSON/YAML แบบ real-time | Must |
